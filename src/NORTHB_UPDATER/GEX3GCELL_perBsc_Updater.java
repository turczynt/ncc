 ...
 * @author turczyt
 */
public class GEX3GCELL_perBsc_Updater extends Updater_parrent
{
     public GEX3GCELL_perBsc_Updater(String identyfikator,String bscName,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
	...
	try
	{
		...
		lstBts=north.make(this.kontrolerName, "LST GEXT3GCELL:;");

		if(lstBts!=null&&lstBts.contains("RETCODE = 0"))
		{
		    //sukces = true;
		    System.out.println(this.identyfikator+" LST GEXT3GCELL: wykonane");
		    this.errorInfo=this.errorInfo+"; POBRANE LST GEXT3GCELL:";
		}
		else
		{
		    System.out.println(this.identyfikator+" LST GEXTGCELL: ERROR");
		...
		if(lstBts.contains("RETCODE = 0"))
		{
			...
			for(int u=0;u<relacje2g2g.size();u++)
			{
				...
				//`external_rnc_bsc_index_FK` INT(11) NULL DEFAULT NULL,
                                ...
                                String Scrambling_code=relacja.getWartosc("Scrambling Code or Cell Parameter Id");
                                String Route_Area=relacja.getWartosc("Route Area");
                                String UARFCN=relacja.getWartosc("DL UARFCN");
                                String oryginal_ucell_rnc_id="'"+relacja.getWartosc("RNC ID")+"'";
                                String oryginal_ucell_index_FK="(select g.Cell_Index from oncall.konfiguracja_aktualna_ucell g where g.Cell_Id="+Ci_dec+" and g.Lac_dec="+lac_dec+" limit 1)";
                                String oryginal_ucell_rnc_rnc_index_FK="(select b.Rnc_Bsc_Index from oncall.konfiguracja_aktualna_nodeb b where b.Nodeb_Index=(select g.Nodeb_Index from oncall.konfiguracja_aktualna_ucell g where  g.Cell_Id="+Ci_dec+" and g.Lac_dec="+lac_dec+" limit 1))";
                                
                                
                                 String insert="INSERT INTO `raport_konfiguracja_aktualna`.`GEXT3GCELL_perBsc` "
                                         + "("
                                         ...
                                         + " ON DUPLICATE KEY UPDATE "
                                            ...
                                            + "UARFCN="+UARFCN+","
                                            + "oryginal_ucell_rnc_id="+oryginal_ucell_rnc_id+","
     ...
     public boolean clearNotUpdatet()
    {
	 ...
	 try
	 {
	     ...
	     String obecnyDzien="'"+sdf.format(DataDzisiaj)+"'";
             String query="delete from raport_konfiguracja_aktualna.GEXT3GCELL_perBsc where last_update<"+obecnyDzien+" and external_rnc_bsc_index_FK="+bsc_index+";";
	...
	try
	{
	    String req="select * from `raport_konfiguracja_aktualna`.`GEXT3GCELL_perBsc` bt where(bt.last_update<"+obecnyDzien+" and bt.external_rnc_bsc_index_FK='"+bsc_index+"');";
	    ResultSet res=testStatement.executeQuery(req);
	    OdpowiedzSQL rnc=Baza.createAnswer(res);
	    return rnc;
	}
