 ...
 * @author turczyt
 */
public class NodebCMCTRL_perRnc_Updater extends Updater_parrent
{
    String rnc_index;
    public NodebCMCTRL_perRnc_Updater(String identyfikator,String kontroler,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
		...
		if(rnc.rowCount()>0)
		{
			...
			try
			{
			    ...
			    String neIndex=nody.getValue("Ne_index", i);
                            String lst="";
                            lst=north.make(NE_Name, "LST CMCTRL:");
				...
				{
				    	npack = new NPack(lst.split("\n"), new String[]{"LST CMCTRL:"}, new String[]{"----------"}, new String[]
						    {
							"---    END"
						    });
					update_stat="'OK'";
					...
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
                                            cmctrl_stat="'"+pack.getWartosc("CM Control State")+"'";
					}
				    
				    
                                    if(cmctrl_stat.equals("''"))
				...
				String insert="insert into raport_konfiguracja_aktualna.nodeb_params_northb (rowAuto_inc,nodeb_index,cmctrl_state,create_date,last_update_date,Update_Status) values (null,'"+neIndex+"',"+cmctrl_stat+","+obecnyDzienCzas+","+obecnyDzienCzas+","+update_stat+")ON DUPLICATE KEY UPDATE last_update_date="+obecnyDzienCzas+",Update_Status="+update_stat+", cmctrl_state="+cmctrl_stat;
