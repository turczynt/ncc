 ...
 * @author turczyt
 */
public class NodebIsdSwitch_perRnc_Updater extends Updater_parrent
{
    String rnc_index;
    public NodebIsdSwitch_perRnc_Updater(String identyfikator,String kontroler,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
			...
			try
			{
			  ...
			  //  System.out.println("NE_Name="+NE_Name);
			   ...
			   if(nodebType.contains("3900"))
				lst=north.make(NE_Name, "LST NODEBPOWEROUTAGE:");
			   else
			       lst=north.make(NE_Name, "LST ITELSHUTDOWN:");
				...
				else
				{
				    if(nodebType.contains("3900"))
				    {
					npack = new NPack(lst.split("\n"), new String[]{"LST NODEBPOWEROUTAGE:"}, new String[]{"----------"}, new String[]
						    {
							"---    END"
						    });
					update_stat="'OK'";
					...
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
                                            isd_switch="'"+pack.getWartosc("Intelligent Shutdown Switch")+"'";
					}
				    }
				    else
				    {

					npack = new NPack(lst.split("\n"), new String[]{"LST ITELSHUTDOWN:"}, new String[]{"----------"}, new String[]{"---    END"});
					update_stat="'OK'";
					java.util.ArrayList<Paczka> mcgrLst=npack.getAllPacks();
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
					    isd_switch="'"+pack.getWartosc("NodeB ISD Switch")+"'";
					}
				    }
				}
                                if(isd_switch.equals("''"))
                                    update_stat="'"+nody.getValue("Status", i)+"'";
				String insert="insert into raport_konfiguracja_aktualna.nodeb_params_northb (rowAuto_inc,nodeb_index,isd_switch,create_date,last_update_date,Update_Status) values (null,'"+neIndex+"',"+isd_switch+","+obecnyDzienCzas+","+obecnyDzienCzas+","+update_stat+")ON DUPLICATE KEY UPDATE last_update_date="+obecnyDzienCzas+",Update_Status="+update_stat+", isd_switch="+isd_switch;
