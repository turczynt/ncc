 ...
 * @author turczyt
 */
public class NodebMntMode_perRnc_Updater extends Updater_parrent
{
    String rnc_index;
    public NodebMntMode_perRnc_Updater(String identyfikator,String kontroler,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
			...
			try
			{
			  ...
			  //  System.out.println("NE_Name="+NE_Name);
			   ...
			   if(nodebType.contains("3900"))
				lst=north.make(NE_Name, "LST MNTMODE:");
			   else
			       lst=north.make(NE_Name, "LST NODEBSTATUS:");
				...
				else
				{
				    if(nodebType.contains("3900"))
				    {
					npack = new NPack(lst.split("\n"), new String[]{"LST MNTMODE:"}, new String[]{"----------"}, new String[]
						    {
							"---    END"
						    });
					update_stat="'OK'";
					...
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
                                            ne_stat="'"+pack.getWartosc("Maintenance Mode")+"'";
					}
				    }
				    else
				    {
					...
					{
					    Paczka pack=mcgrLst.get(0);
					    ne_stat="'"+pack.getWartosc("NE Status")+"'";
				...
				String insert="insert into raport_konfiguracja_aktualna.nodeb_params_northb (rowAuto_inc,nodeb_index,ne_status,create_date,last_update_date,Update_Status) values (null,'"+neIndex+"',"+ne_stat+","+obecnyDzienCzas+","+obecnyDzienCzas+","+update_stat+")ON DUPLICATE KEY UPDATE last_update_date="+obecnyDzienCzas+",Update_Status="+update_stat+", ne_status="+ne_stat;
