 ...
 * @author turczyt
 */
public class Nodeb_adaRetSwitch_perRnc_Updater extends Updater_parrent
{
    String rnc_index;
    public Nodeb_adaRetSwitch_perRnc_Updater(String identyfikator,String kontroler,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
			...
			try
			{
			  ...
			  //  System.out.println("NE_Name="+NE_Name);
			   ...
			   if(nodebType.contains("3900"))
				lst=north.make(NE_Name, "LST NODEBADPRETRANSSW:");
			   else
			       lst=north.make(NE_Name, "LST ADPRETRANSSWTCH:");
				//System.err.println(lst);
				NPack npack=null;

				//No matching result is found
                                String ada_retrans_switch="''";
				String update_stat="''";
				...
				else
				{
				    if(nodebType.contains("3900"))
				    {
					npack = new NPack(lst.split("\n"), new String[]{"LST NODEBADPRETRANSSW:"}, new String[]{"----------"}, new String[]
						    {
							"---    END"
						    });
					update_stat="'OK'";
					...
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
                                            ada_retrans_switch="'"+pack.getWartosc("HSUPA AdaRetransSwitch")+"'";
					}
				    }
				    else
				    {

					npack = new NPack(lst.split("\n"), new String[]{"LST ADPRETRANSSWTCH"}, new String[]{"----------"}, new String[]{"---    END"});
					update_stat="'OK'";
					java.util.ArrayList<Paczka> mcgrLst=npack.getAllPacks();
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
					    ada_retrans_switch="'"+pack.getWartosc("Adaptive Retransmission Control Switch")+"'";
					}
				    }
				}
                                if(ada_retrans_switch.equals("''"))
                                    update_stat="'"+nody.getValue("Status", i)+"'";
				String insert="insert into raport_konfiguracja_aktualna.nodeb_params_northb (rowAuto_inc,nodeb_index,ada_retrans_switch,create_date,last_update_date,Update_Status) values (null,'"+neIndex+"',"+ada_retrans_switch+","+obecnyDzienCzas+","+obecnyDzienCzas+","+update_stat+")ON DUPLICATE KEY UPDATE last_update_date="+obecnyDzienCzas+",Update_Status="+update_stat+", ada_retrans_switch="+ada_retrans_switch;
