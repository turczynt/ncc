 ...
 * @author turczyt
 */
public class Nodeb_dlflowcontrolpara_perRnc_Updater extends Updater_parrent
{
    String rnc_index;
    public Nodeb_dlflowcontrolpara_perRnc_Updater(String identyfikator,String kontroler,int typOperacji,Logger loger,mysqlpackage.DataSource DOA,NewFile sprzF)
    {
	...
	sukces=true;
		...
		if(rnc.rowCount()>0)
		{
		    ...
		    String nodReq="select nt.NodebName as NE_Name,nt.Ne_index,nt.Status, nn.Nodeb_Name as Nodeb_Name_On_Rnc, nn.NodebType, r.Rnc_Bsc_Name,m.M2000_Ip  from oncall.konfiguracja_aktualna_NodebType nt, oncall.konfiguracja_aktualna_nodeb nn, oncall.konfiguracja_aktualna_m2000 m, oncall.konfiguracja_aktualna_rnc_bsc r where (nn.Rnc_Bsc_Index='"+rnc_index+"' and nn.Nodeb_Index=nt.Ne_index and nn.Rnc_Bsc_Index=r.Rnc_Bsc_Index and m.M2000_Index =r.M2000_Index)";
			...
			try
			{
			  ...
			  //  System.out.println("NE_Name="+NE_Name);
			   ...
			   if(nodebType.contains("3900"))
				lst=north.make(NE_Name, "LST DLFLOWCTRLPARA:");
			   else
			       lst=north.make(NE_Name, "LST HSDPAFLOWCTRLPARA:");
				//System.err.println(lst);
				NPack npack=null;

				//No matching result is found
                                String FlowControlSwitch  ="''";
                                String timeDelay  ="''";
				String update_stat="''";
				...
				if(lst.contains("No matching result is found"))
				{
				     update_stat="'No matching result is found'";
                                     timeDelay  ="'No matching result is found'";
                                     update_stat="'No matching result is found'";
				}
				...
				else
				{
				    if(nodebType.contains("3900"))
				    {
					npack = new NPack(lst.split("\n"), new String[]{"LST DLFLOWCTRLPARA:"}, new String[]{"----------"}, new String[]
						    {
							"---    END"
						    });
					update_stat="'OK'";
					...
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
                                            FlowControlSwitch="'"+pack.getWartosc("Flow Control Switch")+"'";
                                            timeDelay="'"+pack.getWartosc("Time Delay(5ms)")+"'";
					}
				    }
				    else
				    {

					npack = new NPack(lst.split("\n"), new String[]{"LST HSDPAFLOWCTRLPARA:"}, new String[]{"----------"}, new String[]{"---    END"});
					update_stat="'OK'";
					java.util.ArrayList<Paczka> mcgrLst=npack.getAllPacks();
					if(mcgrLst.size()>0)
					{
					    Paczka pack=mcgrLst.get(0);
					    FlowControlSwitch="'"+pack.getWartosc("Hsdpa Switch")+"'";
                                            timeDelay="'"+pack.getWartosc("Time Delay(5ms)")+"'";
					}
				    }
				}
                                if(FlowControlSwitch.equals("''")||timeDelay.equals("''"))
                                    update_stat="'"+nody.getValue("Status", i)+"'";
				String insert="insert into raport_konfiguracja_aktualna.nodeb_params_northb (rowAuto_inc,nodeb_index,flow_control_switch,time_delay,create_date,last_update_date,Update_Status) values (null,'"+neIndex+"',"+FlowControlSwitch+","+timeDelay+","+obecnyDzienCzas+","+obecnyDzienCzas+","+update_stat+")ON DUPLICATE KEY UPDATE last_update_date="+obecnyDzienCzas+",Update_Status="+update_stat+", flow_control_switch="+FlowControlSwitch+", time_delay="+timeDelay;
